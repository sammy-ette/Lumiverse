...
fn placeholder_page() {
  html.main(
    ...
    [html.span([attribute.class("icon-circle-o-notch animate-spin")], [])],
  )
}
...
pub fn page(model: model.Model) -> element.Element(layout.Msg) {
  let res = {
    ...
    use roles <- result.try(option.to_result(user.roles, placeholder_page()))
    use <- bool.guard(
      bool.negate(roles |> list.contains(auth.Admin)),
      Error(error.page(lustre_http.OtherError(403, "Forbidden"))),
    )
    use <- bool.guard(model.libraries |> list.is_empty, Ok(placeholder_page()))
    Ok(
      html.main(
        [
          attribute.class(
        ...
        [
          html.div(
            ...
            [
              ...
              html.span(
                [attribute.class("space-x-2")],
                case model.upload_result {
                  option.None -> []
                  option.Some(Error(_)) -> [
            ...
            [
              html.div([], [
                html.label(
                  [
                    attribute.class(
                      "block text-white text-md font-semibold mb-2",
                    ),
                  ],
                ...
                html.select(
                  ...
                  list.map(model.libraries, fn(lib: library.Library) {
                    html.option(
                      [attribute.value(int.to_string(lib.id))],
                      lib.name,
                    )
                  }),
                ),
