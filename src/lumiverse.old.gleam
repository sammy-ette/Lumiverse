...
pub fn main() {
  ...
  let assert Ok(_) = reader_page.register()
...
fn init(_) {
  ...
  let route = router.uri_to_route(router.get_route())
...
fn on_url_change(uri: uri.Uri) -> layout.Msg {
  router.uri_to_route(uri) |> router.ChangeRoute |> layout.Router
}
      ...
      #(
        ...
        effect.batch(list.append(
          ...
          list.append(
            ...
            [
              route_effect(model, router.uri_to_route(router.get_route())),
            ],
          ),
        )),
      )
  ...
  case model.health_failed {
    option.Some(False) -> {
      let page = case model.route {
        ...
        router.Reader(_) -> reader_page.element()
